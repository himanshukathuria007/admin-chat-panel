...
import { IUser, IMessage,IPMessage } from "src/assets/interfaces/shared.interface";
...
export class ChatAppComponent implements OnInit {
  ...
  public currentprivateUser: IUser;
  public usersList: IUser[] = [];
  public messages: IMessage[] = [];
  public pmessages: IPMessage[] = [];
  ...
  private initSocketListener() {
    ...
    this.socket.on("get personal messages history", (pmessages: string) => {
      const historyMessages = [...JSON.parse(pmessages)];
      this.pmessages = historyMessages.map(pmessage => {
        return {
          ...pmessage,
          userName: this.getUserNameById(pmessage.userId)
        };
      });
      this.cdr.markForCheck();
    });
    ...
    this.socket.on("message", (pmessage: string) => {
